...
fn run_encrypt(cli: &CliArgs, files: &[PathBuf]) -> Result<(), ValvError> {
    ...
    let mut password = read_password(cli)
    ...
    let res = (|| -> Result<(), ValvError> {
        for file_path in files {
            ...
            if cli.to_stdout {
                ...
                if let Err(e) = decrypt_file_to(file_path, &password, &mut stdout) {
                    return Err(match e {
                ...
                let mut header = match decrypt_header(&mut reader, &password) {
                    Ok(h) => h,
                ...
                let orig_name = valv::vault::sanitize_filename(&header.original_name);
...
fn run() -> Result<(), ValvError> {
    ...
    match mode {
        ...
        Mode::SyncDaemon => {
            ...
            let res = run_sync_daemon(vault_dir, mount_dir, &password, cli.watch_pid, iterations);
            valv::crypto::zeroize(&mut password);
            res?;
        }
